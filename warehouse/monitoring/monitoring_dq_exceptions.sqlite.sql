-- SQLite-friendly exception report. Uses the same CTE pattern but swaps in datetime()/trim().
...
bad_rest_fk AS (
  -- LEFT JOIN + IS NULL identifies missing restaurant references.
...
bad_cour_fk AS (
  -- Courier referential integrity guard, skipping NULL courier assignments.
...
unknown_status AS (
  -- Flag any status outside the accepted delivered/canceled/returned list.
